...
Serves the repo root on `http://127.0.0.1:8766` with `GET /api/books`, `GET /api/status?book=<id>`, `GET /api/run`, `POST /api/run` (JSON `{ "book", "stage" }` → allowlisted `python main.py …` subprocess), and `POST /api/handoff-answers` (JSON `{ "book", "answers" }` → write `state/<id>/book-visual-handoff-answers.json`), and opens `web/pipeline.html?book=<id>`. Pick a catalog book to see stage badges (`done` / `partial` / `missing`), copy-paste CLI hints, or **Run** a runnable stage. Progress is polled via `/api/run` plus filesystem status refresh; one job at a time (`409` if busy). Artifacts: `state/<id>/run-status.json` and `pipeline-run.log`. Illustrations and handoff-answers are not runnable from the UI. When handoff JSON exists, **Open handoff** opens same-origin `web/handoff.html?book=<id>` on this port (**Save to state** or Download). If port 8766 already serves `/api/books`, re-running only reopens the browser. Stops with Ctrl+C when this process owns the server. Dedicated handoff entry remains `view-handoff` on port 8765; the same APIs exist on that shared handler.
...
7. **Illustrations** and **Handoff answers** rows — expect no **Run** button (Copy CLI / **Open handoff** only).
8. With Alice handoff JSON present, **Open handoff** — expect same-origin `web/handoff.html?book=alice-wonderland` on port **8766**, open questions listed; **Save to state** / Download available. Dedicated `view-handoff` on 8765 still works.
